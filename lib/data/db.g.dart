...
class Wallet extends DataClass implements Insertable<Wallet> {
  ...
  /// Balance before any recorded transaction. The current balance is never
  /// stored — see the `wallet_balances` view.
  final int initialBalanceMinor;

  /// ARGB value.
  final int color;

  /// Key into the app's icon map, not a font code point, so icon sets can
  /// change without a migration.
  final String icon;

  /// Archived wallets keep their history but drop out of pickers and totals.
  final bool archived;
  final int sortOrder;
...
class RecurringRule extends DataClass implements Insertable<RecurringRule> {
  ...
  /// Transfers are not schedulable; only income and expense.
  final String type;
  ...
  /// Day of month the rule was created for, 1–31.
  ///
  /// Kept separately from [nextDate] so month-end rules do not drift: a rule
  /// anchored on the 31st clamps to the 28th in February but returns to the
  /// 31st in March, whereas repeatedly adding a month to the last generated
  /// date walks the rule forward a few days every year.
  final int anchorDay;

  /// Next occurrence, `YYYY-MM-DD`.
  final String nextDate;

  /// Inclusive last date, `YYYY-MM-DD`. Null means it runs forever.
...
class Transaction extends DataClass implements Insertable<Transaction> {
  ...
  /// Always positive. Direction comes from [type], which keeps the balance
  /// view's arithmetic explicit instead of hiding it in a sign.
  final int amountMinor;

  /// Copied from the wallet at write time so historical rows stay correct even
  /// if the wallet's currency is later changed.
  final String currency;
  final String? note;

  /// Calendar day as `YYYY-MM-DD`.
  ///
  /// Deliberately text, not a timestamp: every grouping the app does is by
  /// local calendar day or month, and text dates make that a plain string
  /// comparison in SQL with no timezone ambiguity at boundaries.
  final String date;

  /// Destination wallet — set if and only if [type] is
  ...
  /// Maintained on every write. Not used yet; it is what would make optional
  /// end-to-end-encrypted sync possible later without a migration.
  final DateTime updatedAt;
  const Transaction({
    required this.id,
    required this.walletId,
...
class Budget extends DataClass implements Insertable<Budget> {
  ...
  /// Null means an overall budget covering every expense category.
  final int? categoryId;
  ...
  /// The display currency at the time the budget was set.
  final String currency;

  /// Highest alert threshold already notified for this budget, as a percentage
  /// (0, 80, 100). Stops the 80% notification firing again on every subsequent
  /// expense in the same month.
...
class ExchangeRate extends DataClass implements Insertable<ExchangeRate> {
  ...
  /// Null for a hand-entered rate; set when fetched.
